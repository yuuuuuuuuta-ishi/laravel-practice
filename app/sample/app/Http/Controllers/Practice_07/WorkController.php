...
use App\Http\Requests\Practice_07\WorkRequest;
use App\Http\Requests\Practice_07\GetWorkInfoRequest;
use App\Models\User;
...
class WorkController extends Controller
{
    ...
    public function index(GetWorkInfoRequest $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return response()->view('practice_07.home',['responseData' => $responseData, 'validations' => $validations], 422);
        }

        //users存在判定
        if (self::isExistUser($employeeCode)) {
            $errorMessage = '対象のユーザーが存在しません';
            return response()->view('practice_07.home', ['responseData' => $responseData, 'errorMessage' => $errorMessage], 400);
        }
