...
use App\Http\Requests\Practice_08\WorkRequest;
use App\Http\Requests\Practice_08\GetWorkInfoRequest;
use App\Models\User;
...
class WorkController extends Controller
{
    ...
    public function index(GetWorkInfoRequest $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return response()->view('practice_08.home',['responseData' => $responseData, 'validations' => $validations], 422);
        }

        //users存在判定
        if (self::isExistUser($employeeCode)) {
            $errorMessage = '対象のユーザーが存在しません';
            return response()->view('practice_08.home', ['responseData' => $responseData, 'errorMessage' => $errorMessage], 400);
        }

        //入力値で勤怠情報を取得
        log::info($month);
        $workInfo = DayWorkInformation::getMonthWorkInfo($employeeCode, $month)
        ->withPath('/practice_08/work/get');
        ...
        return view('practice_08.work', ['responseData' => $responseData]);
    }
    ...
    public function store(WorkRequest $request)
    {
        ...
        if ($validator->fails()) {
            $validations =  json_decode((string)$validator->getMessageBag(), true);
            return response()->view('practice_08.work',['responseData' => $responseData, 'validations' => $validations], 422);
        }
        ...
        if (self::isExistUser($employeeCode)) {
            $errorMessage = '対象のユーザーが存在しません';
            return response()->view('practice_08.work', ['responseData' => $responseData, 'errorMessage' => $errorMessage], 400);
        }
